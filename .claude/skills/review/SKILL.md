...
### Step 5: Debate Protocol (default ON, skip with `--no-debate`)

If `--no-debate` flag is passed, skip to Step 6 and use preliminary merged findings.

**Prerequisite:** At least 2 reviewers must have completed. If only 1 reviewer available, skip debate.

#### Step 5a: Preliminary Merge

Collect all findings, deduplicate by file + problem, tag by source.

#### Step 5b: Challenge Round
