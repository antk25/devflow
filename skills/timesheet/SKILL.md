...
## Зеркало задачи SE
Взял задачу SE в работу — сразу заведи ей зеркало GS, чтобы черновик табеля `employer` не ставил
пометку «зеркала нет»:
```bash
~/.claude/skills/timesheet/timesheet mirror SE-<n>          # dry run: найденное зеркало или тело создания
~/.claude/skills/timesheet/timesheet mirror SE-<n> --yes    # создать GS «SE-<n> <название SE>», исполнитель — ты
```
Зеркало ищется по началу названия `SE-<n>`; найдено — ничего не создаётся, повторный вызов вернёт
тот же ключ. Два кандидата — отказ (`ambiguous-mirror`), разобраться вручную. Запуск с `--yes` стоит
под ask-правилом.
