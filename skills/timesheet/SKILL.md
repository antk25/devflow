...
тот же ключ. Два кандидата — отказ (`ambiguous-mirror`), разобраться вручную. Созданное зеркало
записывается в журнал `mirrors.jsonl` рядом с `sent.jsonl`: повторный вызов берёт ключ оттуда, не дожидаясь
поиска Jira. Если поиск по GS упёрся в лимит 5000 задач, создание отменяется — список мог обрезаться.
Запуск с `--yes` стоит под ask-правилом.
