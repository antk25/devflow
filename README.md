...
- **Тройное код-ревью** — Claude + Qwen + ChatGPT параллельно, мерж с рейтингом уверенности
...
|----------|---------|--------|
| Новая фича | `/develop` | Полный пайплайн: план, реализация, тройное ревью — останавливается на work-ветке |
...
**Пайплайн:**
```
ветка → [deep trace] → план → [контракт → ревью в Obsidian] → [тесты из контракта] → реализация → валидация архитектуры → E2E тесты → тройное ревью (Claude + Qwen + ChatGPT) → сохранение знаний → СТОП
```
...
Завершает контекст текущей задачи: сохраняет заметки, обновляет очередь. Проект остаётся активным — можно сразу начать следующую задачу.
...
/queue run --background          # Запустить в фоновом tmux
/queue stop                      # Остановить фоновый запуск
/queue status                    # Результаты
/queue report                    # Утренний отчёт
/queue report --save             # Сохранить отчёт в Obsidian
```

**Фоновый режим:** запуск в отдельной tmux-сессии для выполнения пакета задач на ночь. По завершении генерируется утренний отчёт с результатами, ветками и ошибками. Десктоп-уведомление (`notify-send`) после каждой задачи.
...
devflow-status queue             # Только очередь
devflow-status recent 5          # Последние 5 сессий
watch -n2 devflow-status         # Авто-обновление каждые 2 сек
```

Показывает активную сессию (прогресс-бар фазы, длительность, циклы), статус очереди и недавние сессии. Bash + встроенный Python, без внешних зависимостей.
...
Компактная строка для tmux (~50 символов): `⏳ /develop Implement 47% [Q:2/5]`. Без ANSI-цветов.
...
| **Reviewer** | Код-ревьюер (Claude + Qwen + ChatGPT) | Тройное ревью параллельно, мерж с рейтингом уверенности |
...
**Шаблоны агентов** (`.claude/agents/templates/`) содержат универсальные правила (качество тестов, безопасность, архитектурное соответствие), которые применяются ко всем проектам. Проектные агенты расширяют их специфичными для стека знаниями.
...
| `/queue` | (по задаче) | (по задаче) | Делегирует вызванному скиллу |
...
│   ├── queue-bg.sh                  # Фоновый запуск очереди (tmux)
│   └── queue-report.sh              # Утренний отчёт по очереди
├── mcp-servers/
