#!/usr/bin/env bash
# jira-jql.sh — read-only постраничный JQL-поиск по Jira (аккаунт — --account, без него — по умолчанию).
...
# Только GET, креды — через jira-accounts.sh. Ничего не пишет.
# Использование: jira-jql.sh [--account <имя>] '<JQL>' [fields] [maxTotal] [expand]
set -euo pipefail
